...
pub use app::App;
pub use config::{
    startup_output_channels, startup_output_verbosity, OutputVerbosity, RuntimeConfig,
};
...
pub fn advance_field_sprint(
    ...
    state.player_position[0] =
        (state.player_position[0] + horizontal * speed * delta_seconds).clamp(-0.82, 0.82);
    state.player_position[1] =
        (state.player_position[1] + vertical * speed * delta_seconds).clamp(-0.70, 0.70);
...
mod tests {
    ...
    fn field_sprint_sequence_matches_for_native_and_browser_inputs() {
        ...
        let mut native_state = FieldSprintState {
            target_position: FIELD_SPRINT_TARGET_POINTS[0],
            ..Default::default()
        };
    ...
    fn mouse_hold_toggles_accent_without_changing_the_shared_motion_logic() {
        ...
        let mut held_state = FieldSprintState {
            target_position: FIELD_SPRINT_TARGET_POINTS[0],
            ..Default::default()
        };
