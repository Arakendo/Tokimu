...
fn fs_main(@location(0) local_position: vec3<f32>) -> @location(0) vec4<f32> {
    let band = 0.5 + 0.5 * sin(local_position.x * 12.0 + local_position.y * 9.0);
    let glow = clamp(1.0 - length(local_position.xy), 0.0, 1.0);
    let tint = vec3<f32>(0.25 + band * 0.75, 0.35 + glow * 0.55, 0.75 + glow * 0.20);
