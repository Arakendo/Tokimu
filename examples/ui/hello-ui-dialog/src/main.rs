...
fn main() -> PlatformResult<()> {
    run_window_with_app(
        WindowConfig { title: "Tokimu Hello UI Dialog".into(), width: 1260, height: 820 },
...
impl Default for HelloUiDialogApp {
    fn default() -> Self {
        Self { renderer: None, window_size: [1.0, 1.0], pipeline: PipelineHandle(0), theme: UiTheme::default(), open: true, hovered_button: None, active_button: 1 }
    }
}
...
impl PlatformEventHandler for HelloUiDialogApp {
    fn on_native_window_created(&mut self, window: Arc<NativeWindow>) -> PlatformResult<()> {
        ...
        renderer.upload_material(BACKDROP_MATERIAL, &Material::new("ui-dialog-backdrop", Color::rgb(0.05, 0.06, 0.08)))?;
        renderer.upload_material(SURFACE_MATERIAL, &Material::new("ui-dialog-surface", Color::rgb(0.18, 0.20, 0.25)))?;
        renderer.upload_material(PANEL_MATERIAL, &Material::new("ui-dialog-panel", Color::rgb(0.14, 0.16, 0.20)))?;
        renderer.upload_material(CARD_MATERIAL, &Material::new("ui-dialog-card", Color::rgb(0.22, 0.24, 0.30)))?;
        renderer.upload_material(ACTIVE_MATERIAL, &Material::new("ui-dialog-active", Color::rgb(0.34, 0.56, 0.86)))?;
        renderer.upload_material(MUTED_MATERIAL, &Material::new("ui-dialog-muted", Color::rgb(0.10, 0.12, 0.14)))?;
        self.pipeline = renderer.register_pipeline(&Pipeline::new("hello-ui-dialog-pipeline", PipelineKind::SolidColor2d))?;
    ...
    fn on_platform_event(&mut self, event: PlatformInputEvent) -> PlatformResult<()> {
        match event {
            ...
            PlatformInputEvent::CursorMoved { x, y } => {
                ...
                self.hovered_button = if point[0] > 0.42 && point[0] < 0.58 && point[1] > 0.62 && point[1] < 0.76 {
                    Some(0)
    ...
    fn on_frame(&mut self, _delta_seconds: f64) -> PlatformResult<FrameOutcome> {
        ...
        let backdrop = UiSurfaceCommand { rect: UiRect::new([0.0, 0.0], [self.window_size[0] / self.window_size[1], 1.45]), style: self.theme.surface(UiSurfaceRole::Overlay) };
        Self::draw_surface(renderer, self.pipeline, &backdrop);

        if self.open {
            let dialog = UiCard::new(UiCardRole::Inspector, "", "", UiRegion::card(UiRect::new([0.0, 0.08], [0.80, 0.64])));
            let dialog_rect = UiSurfaceCommand { rect: dialog.region.rect, style: self.theme.card(UiCardRole::Inspector) };
            Self::draw_surface(renderer, self.pipeline, &dialog_rect);

            let header = UiSurfaceCommand { rect: UiRect::new([0.0, 0.30], [0.68, 0.08]), style: self.theme.surface(UiSurfaceRole::Selected) };
            Self::draw_surface(renderer, self.pipeline, &header);

            let body = UiSurfaceCommand { rect: UiRect::new([0.0, 0.12], [0.62, 0.18]), style: self.theme.surface(UiSurfaceRole::Panel) };
            ...
            for (index, rect) in buttons.iter().enumerate() {
                ...
                let command = UiSurfaceCommand { rect: *rect, style: self.theme.control(if index == 0 { ui_tools::UiControlRole::Secondary } else { ui_tools::UiControlRole::Primary }, if active { ui_tools::UiInteractionState::Hovered } else { ui_tools::UiInteractionState::Idle }) };
                Self::draw_surface(renderer, self.pipeline, &command);
            }
        } else {
            let reopen = UiSurfaceCommand { rect: UiRect::new([0.0, 0.0], [0.42, 0.12]), style: self.theme.surface(UiSurfaceRole::Panel) };
