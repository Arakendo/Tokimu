...
use ui_tools::{
    parse_svg_document_vector_paths, tessellate_path_strokes, window_to_world, VectorPath,
};
...
const GRID_ORIGIN: [f32; 2] = [-0.86, 0.84];
const GRID_CELL_STEP: f32 = 0.19;
...
fn main() -> PlatformResult<()> {
    run_window_with_app(
        WindowConfig {
            title: "Tokimu Hello UI Lucide 2".into(),
            width: 900,
            height: 600,
        },
        App::default(),
...
fn sample_manifest() -> Result<Vec<String>, String> {
    ...
    let path = candidates
        ...
        .ok_or_else(|| "run prepare-lucide-sample.ps1 first".to_string())?;
...
fn sample_svg_paths(name: &str) -> Result<Vec<VectorPath>, String> {
    ...
    let path = candidates
        .into_iter()
        .find(|path| path.is_file())
        .ok_or_else(|| format!("missing sampled icon {name}"))?;
...
impl PlatformEventHandler for App {
    fn on_native_window_created(&mut self, window: Arc<NativeWindow>) -> PlatformResult<()> {
        ...
        renderer.upload_material(
            MATERIAL,
            &Material::new("lucide2", Color::rgb(0.45, 0.68, 0.92)),
        )?;
        self.meshes.clear();
        let sampled = sample_manifest()
            .map_err(|error| error.to_string())?
            .into_iter()
            .take(100)
            .collect::<Vec<_>>();
        ...
        for (index, name) in sampled.into_iter().take(100).enumerate() {
            ...
            let positions = tessellate_path_strokes(&paths, LUCIDE_STROKE_HALF_WIDTH);
            renderer.upload_mesh(handle, &Mesh::uniform_normal(positions, [0.0, 0.0, 1.0]));
            tessellated_icons += 1;
            self.meshes.push(Some((handle, cell, GRID_ICON_SCALE, 0.0)));
        }
        println!(
            "hello-ui-lucide2 corpus build: icons={}, geometry_ms={:.3}",
            tessellated_icons,
            geometry_start.elapsed().as_secs_f64() * 1000.0
        );
        self.pipeline =
            renderer.register_pipeline(&Pipeline::new("lucide2", PipelineKind::SolidColor2d))?;
    ...
    fn on_platform_event(&mut self, event: PlatformInputEvent) -> PlatformResult<()> {
        ...
        } = event
        {
            ...
            let column = ((world_x - GRID_ORIGIN[0] + GRID_CELL_HALF_EXTENT) / GRID_CELL_STEP)
                .floor() as i32;
            let row = ((GRID_ORIGIN[1] - world_y + GRID_CELL_HALF_EXTENT) / GRID_CELL_STEP).floor()
                as i32;
            self.selected = if (0..10).contains(&column)
                && (0..10).contains(&row)
                && world_x
                    >= GRID_ORIGIN[0] + column as f32 * GRID_CELL_STEP - GRID_CELL_HALF_EXTENT
                && world_x
                    <= GRID_ORIGIN[0] + column as f32 * GRID_CELL_STEP + GRID_CELL_HALF_EXTENT
                && world_y <= GRID_ORIGIN[1] - row as f32 * GRID_CELL_STEP + GRID_CELL_HALF_EXTENT
                && world_y >= GRID_ORIGIN[1] - row as f32 * GRID_CELL_STEP - GRID_CELL_HALF_EXTENT
            {
    ...
    fn on_frame(&mut self, _delta_seconds: f64) -> PlatformResult<FrameOutcome> {
        ...
        let commands = self
            ...
            .filter_map(|(index, cell)| {
                ...
                let draw_scale = if self.selected == Some(index) {
                    [0.8, 0.8]
                } else {
                    *scale
                };
