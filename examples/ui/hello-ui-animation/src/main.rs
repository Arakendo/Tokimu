...
fn main() -> PlatformResult<()> {
    run_window_with_app(
        WindowConfig {
            title: "Tokimu Hello UI Animation".into(),
            width: 1260,
            height: 820,
        },
...
impl PlatformEventHandler for HelloUiAnimationApp {
    fn on_native_window_created(&mut self, window: Arc<NativeWindow>) -> PlatformResult<()> {
        ...
        renderer.upload_material(
            BACKDROP_MATERIAL,
            &Material::new("ui-animation-backdrop", Color::rgb(0.05, 0.06, 0.08)),
        )?;
        renderer.upload_material(
            SURFACE_MATERIAL,
            &Material::new("ui-animation-surface", Color::rgb(0.18, 0.20, 0.25)),
        )?;
        renderer.upload_material(
            PANEL_MATERIAL,
            &Material::new("ui-animation-panel", Color::rgb(0.14, 0.16, 0.20)),
        )?;
        renderer.upload_material(
            CARD_MATERIAL,
            &Material::new("ui-animation-card", Color::rgb(0.22, 0.24, 0.30)),
        )?;
        renderer.upload_material(
            ACTIVE_MATERIAL,
            &Material::new("ui-animation-active", Color::rgb(0.34, 0.56, 0.86)),
        )?;
        renderer.upload_material(
            MUTED_MATERIAL,
            &Material::new("ui-animation-muted", Color::rgb(0.10, 0.12, 0.14)),
        )?;
        self.pipeline = renderer.register_pipeline(&Pipeline::new(
            "hello-ui-animation-pipeline",
    ...
    fn on_platform_event(&mut self, event: PlatformInputEvent) -> PlatformResult<()> {
        match event {
            ...
            PlatformInputEvent::CursorMoved { x, y } => {
                ...
                self.hovered =
                    point[0] > 0.33 && point[0] < 0.67 && point[1] > 0.28 && point[1] < 0.72;
            }
    ...
    fn on_frame(&mut self, delta_seconds: f64) -> PlatformResult<FrameOutcome> {
        ...
        let card = UiCard::new(
            ...
            "",
            "",
            UiRegion::card(UiRect::new([card_x, card_y], [card_width, card_height])),
        );
        ...
        if self.hovered {
            let hover = UiSurfaceCommand {
                rect: UiRect::new([0.0, 0.54], [0.32, 0.06]),
