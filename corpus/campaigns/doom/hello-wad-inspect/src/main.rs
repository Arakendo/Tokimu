...
struct ZipInspection<'a> {
    ...
    patch: Option<&'a OsString>,
    texture: Option<&'a OsString>,
    flat: Option<&'a OsString>,
...
fn main() -> ExitCode {
    ...
    match arguments.as_slice() {
        ...
        [flag, package, member] if flag == "--zip" => {
            inspect_zip_member(package, member, ZipInspection::default())
        }
            ...
            if flag == "--zip" && artifact_flag == "--map-svg" =>
        {
            inspect_zip_member(
                ...
                ZipInspection {
                    map: Some(map),
                    map_svg_request: Some(MapSvgRequest {
                        output: output.clone(),
...
fn inspect_zip_member(
    ...
    match lower_doom_indexed_image(
        ...
        Ok(lowered) => {
            if texture.is_none() && patch.is_none() && flat.is_none() {
                if let Some(ppm_output) = ppm_output {
                    if let Err(error) =
                        write_ppm(ppm_output, lowered.width, lowered.height, &lowered.pixels)
                    {
