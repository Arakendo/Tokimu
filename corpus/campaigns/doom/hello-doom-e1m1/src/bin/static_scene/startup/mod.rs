...
pub(crate) fn run() -> PlatformResult<()> {
    ...
    let live_audio_requested = args.iter().any(|argument| argument == "--audio");
    ...
    let thing_classification_report = args
        .iter()
        .any(|argument| argument == "--thing-classification-report");
    ...
    let grouped_sky_crossing_parity_report = args
        .iter()
        .any(|argument| argument == "--grouped-sky-crossing-parity-report");
    ...
    let gameplay_snapshot_replay_report = args
        .iter()
        .any(|argument| argument == "--gameplay-snapshot-replay-report");
    ...
    args.retain(|argument| argument != "--audio");
    ...
    args.retain(|argument| argument != "--thing-classification-report");
    ...
    args.retain(|argument| argument != "--grouped-sky-crossing-parity-report");
    ...
    args.retain(|argument| argument != "--gameplay-snapshot-replay-report");
    ...
    if let Some(audit) = scene.source_bounded_surface_audit.as_ref() {
        eprintln!(
            ...
            audit
                ...
                .map(|source| source.record_index)
                .collect::<Vec<_>>(),
            audit.edge_conformance_insertions,
    ...
    if grouped_sky_crossing_parity_report {
        report_grouped_sky_crossing_parity(&scene)?;
        return Ok(());
    }
    ...
    if monster_perception_report {
        report_doom_monster_perception(&scene);
        return Ok(());
    }
    ...
    let thing_combat_states = scene
        ...
        .iter()
        .map(|thing| {
            hello_doom_e1m1::combat::DoomCombatActorState::new(
    ...
    if gameplay_snapshot_replay_report {
        report_gameplay_snapshot_replay(&mut app)?;
        return Ok(());
    }
    run_window_with_app(
        WindowConfig {
