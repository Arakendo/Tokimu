...
impl PlatformEventHandler for HelloUiTextInput {
    fn on_native_window_created(&mut self, window: Arc<NativeWindow>) -> PlatformResult<()> {
        ...
        renderer.upload_material(
            BACKDROP,
            &Material::new("text-input-backdrop", Color::rgb(0.05, 0.06, 0.08)),
        )?;
        renderer.upload_material(
            PANEL,
            &Material::new("text-input-panel", Color::rgb(0.20, 0.23, 0.29)),
        )?;
        renderer.upload_material(
            ACTIVE,
            &Material::new("text-input-active", Color::rgb(0.35, 0.58, 0.86)),
        )?;
        renderer.upload_material(
            TEXT,
            &Material::new("text-input-text", Color::rgb(0.92, 0.94, 0.98)),
        )?;
        renderer.upload_material(
            MUTED,
            &Material::new("text-input-muted", Color::rgb(0.58, 0.64, 0.72)),
        )?;
    ...
    fn on_platform_event(&mut self, event: PlatformInputEvent) -> PlatformResult<()> {
        ...
        match event {
            ...
            PlatformInputEvent::KeyboardInput { key, pressed: true } if self.focused => match key {
