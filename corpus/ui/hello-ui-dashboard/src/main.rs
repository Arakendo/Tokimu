...
use ui_tools::{
    UiButton, UiButtonId, UiButtonSpec, UiCardRole, UiControlRole, UiDrawer, UiRect,
    UiSurfaceCommand, UiSurfaceRole, UiTheme, UiWorkspaceLayout,
};
...
fn main() -> PlatformResult<()> {
    run_window_with_app(
        WindowConfig {
            title: "Tokimu Hello UI Dashboard".into(),
            width: 1360,
            height: 840,
        },
...
impl PlatformEventHandler for HelloUiDashboardApp {
    fn on_native_window_created(&mut self, window: Arc<NativeWindow>) -> PlatformResult<()> {
        ...
        renderer.upload_material(
            BACKDROP_MATERIAL,
            &Material::new("ui-dashboard-backdrop", Color::rgb(0.05, 0.06, 0.08)),
        )?;
        renderer.upload_material(
            SURFACE_MATERIAL,
            &Material::new("ui-dashboard-surface", Color::rgb(0.18, 0.20, 0.25)),
        )?;
        renderer.upload_material(
            PANEL_MATERIAL,
            &Material::new("ui-dashboard-panel", Color::rgb(0.14, 0.16, 0.20)),
        )?;
        renderer.upload_material(
            CARD_MATERIAL,
            &Material::new("ui-dashboard-card", Color::rgb(0.22, 0.24, 0.30)),
        )?;
        renderer.upload_material(
            ACTIVE_MATERIAL,
            &Material::new("ui-dashboard-active", Color::rgb(0.34, 0.56, 0.86)),
        )?;
        renderer.upload_material(
            MUTED_MATERIAL,
            &Material::new("ui-dashboard-muted", Color::rgb(0.10, 0.12, 0.14)),
        )?;
        self.pipeline = renderer.register_pipeline(&Pipeline::new(
            "hello-ui-dashboard-pipeline",
    ...
    fn on_platform_event(&mut self, event: PlatformInputEvent) -> PlatformResult<()> {
        match event {
            ...
            PlatformInputEvent::MouseInput {
                button: tokimu::MouseButton::Left,
                pressed: true,
            } => {
    ...
    fn on_frame(&mut self, _delta_seconds: f64) -> PlatformResult<FrameOutcome> {
        ...
        for (index, button) in buttons.iter().enumerate() {
            ...
            for command in button_surfaces {
                Self::draw_surface(renderer, self.pipeline, &command);
            }
        }
        ...
        for (index, card) in layout.cards.iter().enumerate() {
            let mut card = *card;
            card.role = if index == self.active_card {
