...
fn main() -> PlatformResult<()> {
    run_window_with_app(
        WindowConfig {
            title: "Tokimu Hello UI Icons".into(),
            width: 1260,
            height: 820,
        },
        HelloUiIconsApp::new(),
...
impl PlatformEventHandler for HelloUiIconsApp {
    fn on_native_window_created(&mut self, window: Arc<NativeWindow>) -> PlatformResult<()> {
        ...
        renderer.upload_material(
            BACKDROP_MATERIAL,
            &Material::new("ui-icons-backdrop", Color::rgb(0.05, 0.06, 0.08)),
        )?;
        renderer.upload_material(
            SURFACE_MATERIAL,
            &Material::new("ui-icons-surface", Color::rgb(0.18, 0.20, 0.25)),
        )?;
        renderer.upload_material(
            PANEL_MATERIAL,
            &Material::new("ui-icons-panel", Color::rgb(0.14, 0.16, 0.20)),
        )?;
        renderer.upload_material(
            CARD_MATERIAL,
            &Material::new("ui-icons-card", Color::rgb(0.22, 0.24, 0.30)),
        )?;
        renderer.upload_material(
            ACTIVE_MATERIAL,
            &Material::new("ui-icons-active", Color::rgb(0.34, 0.56, 0.86)),
        )?;
        renderer.upload_material(
            MUTED_MATERIAL,
            &Material::new("ui-icons-muted", Color::rgb(0.10, 0.12, 0.14)),
        )?;
        self.pipeline = renderer.register_pipeline(&Pipeline::new(
            "hello-ui-icons-pipeline",
    ...
    fn on_platform_event(&mut self, event: PlatformInputEvent) -> PlatformResult<()> {
        match event {
            PlatformInputEvent::MouseInput {
                ...
                if let Some(index) = self.hovered_tile {
    ...
    fn on_frame(&mut self, _delta_seconds: f64) -> PlatformResult<FrameOutcome> {
        ...
        let mut text = Vec::new();
        {
            ...
            drawer.surface(&UiRegion::panel(UiRect::new(
                [0.0, 0.0],
                [self.window_size[0] / self.window_size[1], 1.40],
            )));
        }

        for (index, rect) in tiles.iter().enumerate() {
            let tile_role = if index == selected_tile {
                UiSurfaceRole::Selected
            } else if Some(index) == hovered_tile {
            ...
            for command in tile_surfaces {
                ...
                if command.style.role == UiSurfaceRole::Card {
                    command.style.role = tile_role;
                }
                Self::draw_surface(renderer, self.pipeline, &command);
            }

            let icon = UiSurfaceCommand {
                rect: UiRect::new([rect.center[0], rect.center[1] + 0.08], [0.10, 0.10]),
                style: self.theme.surface(tile_role),
                clip: None,
            };
            let caption = UiSurfaceCommand {
                rect: UiRect::new(
                    [rect.center[0], rect.center[1] - 0.10],
                    [rect.size[0] * 0.62, 0.06],
                ),
