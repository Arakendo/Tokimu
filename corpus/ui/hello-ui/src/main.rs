...
const FILTER_ACCENTS: [MaterialHandle; 4] =
    [LIME_MATERIAL, CYAN_MATERIAL, AMBER_MATERIAL, ROSE_MATERIAL];
    ...
    fn new(window_size: [f32; 2], mode: UiDensityMode) -> Self {
        ...
        let header = UiRect::new(
            [
                0.0,
                half_height
                    - if matches!(mode, UiDensityMode::Dense) {
                        0.12
                    } else {
                        0.15
                    },
            ],
            [half_width * 1.76, header_height],
        );
        ...
        let tab_width = ((main_width - 0.18) / 3.0).max(if matches!(mode, UiDensityMode::Dense) {
            0.35
        } else {
            0.42
        });
        let tab_y = half_height
            - if matches!(mode, UiDensityMode::Dense) {
                0.24
            } else {
                0.30
        ...
        let filters = [
            ...
            UiRect::new(
                [left_rail.center[0], 0.42 - filter_step],
                [filter_width, 0.16],
            ),
            UiRect::new(
                [left_rail.center[0], 0.42 - filter_step * 2.0],
                [filter_width, 0.16],
            ),
            UiRect::new(
                [left_rail.center[0], 0.42 - filter_step * 3.0],
                [filter_width, 0.16],
            ),
        ];
...
impl HelloUiApp {
    ...
    fn render_scene(&mut self) -> PlatformResult<FrameOutcome> {
        ...
        let camera =
            Camera::orthographic_2d_with_height(self.window_size[0], self.window_size[1], 2.0);
        ...
        for (index, tab_rect) in layout.tabs.iter().enumerate() {
            ...
            if index == self.active_tab {
                let marker = UiRect::new(
                    [tab_rect.center[0], tab_rect.center[1] + 0.08],
                    [0.08, 0.08],
                );
                draw_marker(
                    &mut commands,
        ...
        for (index, card_rect) in layout.cards.iter().enumerate() {
            ...
            let top_bar = UiRect::new(
                [
                    card_rect.center[0],
                    card_rect.center[1] + card_rect.size[1] * 0.13,
                ],
            ...
            let knob = UiRect::new(
                [
                    card_rect.center[0] - card_rect.size[0] * 0.32,
                    card_rect.center[1] - card_rect.size[1] * 0.14,
                ],
                [0.08, 0.08],
            );
            draw_marker(
            ...
            for meter in 0..3 {
                ...
                let bar = UiRect::new(
                    [
                        card_rect.center[0] - 0.12 + meter as f32 * 0.12,
                        meter_base_y,
                    ],
                    [0.07, bar_height],
                );
        ...
        for index in 0..bar_count {
            ...
            let bar = UiRect::new(
                [
                    bar_left + index as f32 * bar_step,
                    layout.chart.center[1] - layout.chart.size[1] * 0.20 + height * 0.5,
                ],
        ...
        let chart_marker = UiRect::new(
            [
                layout.chart.center[0] + layout.chart.size[0] * 0.36,
                layout.chart.center[1] + layout.chart.size[1] * 0.26,
            ],
            [0.10, 0.10],
        );
        draw_marker(
...
impl PlatformEventHandler for HelloUiApp {
    fn on_native_window_created(&mut self, window: Arc<NativeWindow>) -> PlatformResult<()> {
        ...
        renderer.upload_material(
            BACKDROP_MATERIAL,
            &Material::new("ui-backdrop", Color::rgb(0.06, 0.08, 0.11)),
        )?;
        renderer.upload_material(
            SHADOW_MATERIAL,
            &Material::new("ui-shadow", Color::rgb(0.02, 0.03, 0.05)),
        )?;
        renderer.upload_material(
            SURFACE_MATERIAL,
            &Material::new("ui-surface", Color::rgb(0.11, 0.14, 0.19)),
        )?;
        renderer.upload_material(
            SURFACE_ALT_MATERIAL,
            &Material::new("ui-surface-alt", Color::rgb(0.15, 0.18, 0.25)),
        )?;
        renderer.upload_material(
            CYAN_MATERIAL,
            &Material::new("ui-cyan", Color::rgb(0.30, 0.82, 0.96)),
        )?;
        renderer.upload_material(
            AMBER_MATERIAL,
            &Material::new("ui-amber", Color::rgb(0.96, 0.78, 0.31)),
        )?;
        renderer.upload_material(
            CORAL_MATERIAL,
            &Material::new("ui-coral", Color::rgb(0.95, 0.48, 0.55)),
        )?;
        renderer.upload_material(
            LIME_MATERIAL,
            &Material::new("ui-lime", Color::rgb(0.44, 0.92, 0.62)),
        )?;
        renderer.upload_material(
            ROSE_MATERIAL,
            &Material::new("ui-rose", Color::rgb(0.89, 0.42, 0.78)),
        )?;
        renderer.upload_material(
            WHITE_MATERIAL,
            &Material::new("ui-white", Color::rgb(0.95, 0.96, 0.98)),
        )?;
...
fn draw_panel(
    ...
    let strip = UiRect::new(
        [
            rect.center[0],
            rect.center[1] + rect.size[1] * 0.5 - strip_height * 0.5,
        ],
