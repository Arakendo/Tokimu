import init, {
  AsteroidsSession,
} from "./tokimu_website_asteroids_engine.js";

type Vec2 = { x: number; y: number };
type Mode = "playing" | "paused" | "gameOver";
type Snapshot = {
  schema: number;
  mode: Mode;
  width: number;
  ...
  particles: {
    ...
    size: number;
    kind: "thrust" | "impact" | "ship" | "muzzle" | "wave" | "score";
  }[];
};
type InputFrame = {
  thrust: boolean;
...
const wave = required<HTMLElement>("[data-wave]");
const lives = required<HTMLElement>("[data-lives]");
const combo = required<HTMLElement>("[data-combo]");
const comboWrap = required<HTMLElement>("[data-combo-wrap]");
const curtain = required<HTMLElement>("[data-curtain]");
const curtainKicker = required<HTMLElement>("[data-curtain-kicker]");
const curtainTitle = required<HTMLElement>("[data-curtain-title]");
const curtainDetail = required<HTMLElement>("[data-curtain-detail]");
...
const pointer = { x: 0, y: 0, active: false, firing: false, id: null as number | null };
...
let session: AsteroidsSession | null = null;
let snapshot: Snapshot | null = null;
let animationFrame = 0;
let previousTime = performance.now();
let accumulator = 0;
let seed = 0x746f_6b69;
let inputAbort: AbortController | null = null;
...
function frame(now: number): void {
  ...
  while (accumulator >= FIXED_STEP && steps < MAX_STEPS) {
    snapshot = parseSnapshot(
      session.step(JSON.stringify(consumeInput()), FIXED_STEP),
    );
...
function consumeInput(): InputFrame {
...
function draw(state: Snapshot, time: number): void {
  ...
  context.translate(
    Math.sin(time * 83) * shake,
    Math.cos(time * 71) * shake,
  );
...
function drawField(width: number, height: number, time: number): void {
...
function drawAsteroids(state: Snapshot): void {
...
function drawProjectiles(state: Snapshot): void {
  context.save();
  ...
  for (const projectile of state.projectiles) {
    ...
    context.lineTo(
      position.x - Math.cos(projectile.angle) * 8,
      position.y - Math.sin(projectile.angle) * 8,
    );
    context.stroke();
  }
  context.restore();
...
function drawParticles(state: Snapshot): void {
  context.save();
  ...
  for (let index = 0; index < state.particles.length; index += 1) {
    if (prefersReducedMotion && index % 3 !== 0) continue;
    ...
    const radius = Math.max(
      0.6,
      worldScale(particle.size, state) * motionScale,
    );
    const rgb =
      particle.kind === "thrust"
        ? "76, 231, 219"
        : particle.kind === "ship"
          ? "255, 119, 84"
...
function drawShip(state: Snapshot, time: number): void {
  if (
    state.mode === "gameOver" ||
...
function updateHud(state: Snapshot): void {
...
function installInput(): void {
  ...
  addEventListener("keydown", (event) => {
    ...
    if (!event.repeat && event.code === "KeyP") pulses.pause = true;
    if (!event.repeat && event.code === "Enter") pulses.restart = true;
    keys.add(event.code);
  }, options);
...
function worldToCanvas(position: Vec2, state: Snapshot): Vec2 {
  return {
...
function canvasToWorld(position: Vec2): Vec2 | null {
  ...
  return {
    ...
    y:
      (position.y / canvas.clientHeight) * snapshot.height -
      snapshot.height * 0.5,
  };
}

function worldScale(value: number, state: Snapshot): number {
...
function parseSnapshot(json: string): Snapshot {
  ...
  if (value.schema !== 1) throw new Error(`Unsupported snapshot schema ${value.schema}`);
  return value;
}
