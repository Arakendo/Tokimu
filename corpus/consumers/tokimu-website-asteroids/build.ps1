...
$wasm = Join-Path $root "target/wasm32-unknown-unknown/release/tokimu_website_asteroids_engine.wasm"
...
try {
    ...
    wasm-bindgen $wasm --target web --out-dir $dist --out-name tokimu_website_asteroids_engine
...
Write-Host "Prepared Tokimu website Asteroids template at $dist"
