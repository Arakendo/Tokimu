...
$wasm = Join-Path $root "target/wasm32-unknown-unknown/release/tokimu_website_paint_engine.wasm"
...
try {
    ...
    wasm-bindgen $wasm --target web --out-dir $dist --out-name tokimu_website_paint_engine
    npm run build
...
Write-Host "Prepared Tokimu Paint WASM bindings at $dist"
