//! Alternative B port of the `hello-3d-mono` application shell.
//!
//! The app's math-facing code uses the provider-backed candidate vocabulary.
//! The one camera conversion is retained inside the study-owned adapter.
...
use tokimu_math_study::{
    alternative_b::{Mat4, Vec3},
    hello_3d_mono_adapters::alternative_b_camera,
};
...
fn main() -> PlatformResult<()> {
    run_window_with_app(
        WindowConfig {
            title: "Tokimu Hello 3D Mono (math study B)".into(),
            width: 1280,
            height: 720,
        },
        Hello3dMonoApp::new(),
