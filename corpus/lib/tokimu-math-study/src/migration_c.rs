//! Representative Alternative C migration pressure.
//!
//! Unlike Alternative B, the owned candidate has no provider representation to
//! unwrap. A still-provider-specific renderer must reconstruct its upload
//! value from the owned column array at the adapter boundary.

use crate::alternative_c::{Mat4, Vec3};
...
/// The explicit representation conversion required by a retained provider
/// upload boundary.
#[must_use]
pub(crate) fn provider_upload_matrix(transform: Mat4) -> glam::Mat4 {
    let columns = transform.to_cols_array();
    glam::Mat4::from_cols_array(&columns)
}

/// Constructs the current public renderer camera at the explicit C boundary.
///
/// The facade presently exposes provider `Mat4` fields, so this handoff
/// reconstructs provider matrices for view and projection separately. The
/// candidate camera itself remains provider-free.
#[must_use]
...
mod tests {
    ...
    #[test]
    fn owned_camera_reconstructs_the_provider_upload_value_at_the_adapter() {
        let camera = Camera {
            eye: Vec3::new(0.0, 0.0, 5.0),
            center: Vec3::ZERO,
            up: Vec3::Y,
    ...
    fn renderer_boundary_round_trips_a_candidate_matrix_without_public_leakage() {
        ...
        let restored = Mat4::from_cols_array(&provider_upload_matrix(candidate).to_cols_array());
