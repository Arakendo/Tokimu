...
const canvas = need("[data-canvas]");
const context = needCanvasContext(canvas);
const canvasShell = need("[data-canvas-shell]");
const canvasLabel = need("[data-canvas-label]");
const status = need("[data-status]");
const details = need("[data-details]");
const colorInput = need("[data-color]");
const brushSizeInput = need("[data-brush-size]");
const brushSizeValue = need("[data-brush-size-value]");
const undoButton = need("[data-undo]");
const redoButton = need("[data-redo]");
const zoomInput = need("[data-zoom]");
const zoomValue = need("[data-zoom-value]");
const observationList = need("[data-observations]");
let session;
let tool = "pencil";
let strokePoints = [];
let activePointerId;
...
async function boot() {
    ...
    recordObservation(`WASM startup observed at ${startupMilliseconds.toFixed(1)} ms; timing is diagnostic evidence, not a cross-browser guarantee`);
}
function bindControls() {
    ...
    need("[data-new]").addEventListener("click", async () => {
    ...
    need("[data-reset]").addEventListener("click", () => void reset());
    need("[data-export]").addEventListener("click", downloadPng);
    need("[data-zoom-in]").addEventListener("click", () => setZoom(zoomPercent + 25));
    need("[data-zoom-out]").addEventListener("click", () => setZoom(zoomPercent - 25));
    need("[data-fit]").addEventListener("click", fitPreview);
    brushSizeInput.addEventListener("input", () => setBrushDiameter(Number(brushSizeInput.value)));
    zoomInput.addEventListener("input", () => setZoom(Number(zoomInput.value)));
    need("[data-open]").addEventListener("change", (event) => {
...
async function openFile(file) {
    try {
        ...
        const nextSession = WasmPaintSession.open(new Uint8Array(await file.arrayBuffer()), file.type || fileExtension(file.name));
...
async function beginPointerEdit(event) {
...
function collectPointerEdit(event) {
    if (!isActiveStroke(event))
        return;
    event.preventDefault();
...
async function finishPointerEdit(event) {
    if (!isActiveStroke(event))
        return;
    event.preventDefault();
...
function cancelPointerEdit(event) {
...
function cancelLostPointerEdit(event) {
    if (activePointerId !== event.pointerId)
        return;
...
function drawLivePixelLine(start, end) {
    context.save();
...
function drawLiveBrushStamp(centerX, centerY) {
...
function setBrushDiameter(nextDiameter) {
    brushDiameter = clamp(Math.round(nextDiameter), Number(brushSizeInput.min), Number(brushSizeInput.max));
...
async function handleKey(event) {
    ...
    if (event.target instanceof HTMLInputElement)
        return;
    const shortcut = { b: "pencil", e: "erase", f: "fill", i: "sample" };
...
async function refresh(message, controlJson) {
    if (controlJson)
        JSON.parse(controlJson);
    const preview = JSON.parse(session.preview_observation_json());
    canvas.width = preview.width;
    canvas.height = preview.height;
    canvasLabel.textContent = `Document canvas / ${preview.width} x ${preview.height}`;
    context.putImageData(new ImageData(new Uint8ClampedArray(session.preview_bytes()), preview.width, preview.height), 0, 0);
    applyZoom();
    const observation = JSON.parse(session.observation_json());
...
function updateHistoryControls(observation) {
...
function recordObservation(message) {
    const item = document.createElement("li");
    item.textContent = message;
    observationList.prepend(item);
    while (observationList.children.length > 8)
        observationList.lastElementChild?.remove();
}
...
function needCanvasContext(target) {
