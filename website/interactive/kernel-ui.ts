...
const defaultFrameUrl = new URL(
  "../assets/islands/kernel-ui/index.html",
  scriptUrl,
);
...
window.TokimuIslands.register(
  ...
  async ({ root, config: rawConfig, signal }) => {
    ...
    const config = rawConfig as KernelUiIslandConfig;
    ...
    frame.className = "kernel-ui-frame";
    frame.title = "Tokimu kernel UI resource workbench";
...
function waitForFrame(frame: HTMLIFrameElement, signal: AbortSignal): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const onError = () => { cleanup(); reject(new Error("The kernel UI evidence frame failed to load.")); };
    const onAbort = () => { cleanup(); reject(new DOMException("Kernel UI activation was cancelled.", "AbortError")); };
...
function required<T extends Element>(root: ParentNode, selector: string): T {
  ...
  if (!element) throw new Error(`Kernel UI island requires ${selector}.`);
  return element;
}
