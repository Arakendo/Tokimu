...
| Field | Value |
| --- | --- |
| Status | Active — native/browser sky-omission presentation retained; second-caller pressure remains open |
...
It is a first-party corpus/reference asset covered by the repository MIT
license, is 9,797 bytes and 1024x1024, and has an indexed eight-bit PNG source
with no `tRNS` alpha chunk. The caller deliberately supplies point filtering
and repeat addressing; no PNG metadata or renderer default chooses those
semantics. Both native and browser packages embed the checked-in bytes, so the
fixture requires no network or ambient filesystem access after build.
...
compared through every presentation surface, the packaged browser candidate
still needs a human visual observation, and no second non-Doom caller exists.
It therefore supports only AR-0027 Alternative A experimentation.

## Browser Candidate — 2026-08-13

The existing browser intake workbench now exposes an explicit **Render
diagnostic sky omissions** action. TypeScript supplies only the user gesture
and canvas. Rust/WASM reopens the already retained reviewed package, invokes
the same `prepare_e1m1_sky_diagnostic_flats` classifier as native, decodes the
same checked-in Purple PNG through `raster-image-corpus`, and returns a bounded
observation containing:

```text
diagnostic_sky=<submitted>/<retained>
diagnostic_asset=corpus/assets/PNG/Purple/texture_01.png
diagnostic_reason=intentional-source-sky-omission
```

The ordinary static E1M1 buttons still omit these draws. This is therefore a
caller-selected comparison mode, not automatic renderer fallback. The WASM
target compiled and the release web package was regenerated on 2026-08-13.
Compile and packaging success were initially retained separately from browser
execution evidence.

The subsequent actual browser/WebGPU run retained:
...
opaque=1823/1823
cutouts=0/0
diagnostic_sky=73/73
diagnostic_asset=corpus/assets/PNG/Purple/texture_01.png
diagnostic_reason=intentional-source-sky-omission
backend=browser-webgpu
device=other
adapter=<not reported>
canvas=960x600
```

This closes the browser observation for the opt-in stand-in. It does not claim
pixel identity with native, NVIDIA coverage, automatic fallback, or that the
browser adapter name was observable. The semantic claim is narrower: the same
retained omission class and caller-selected asset reached an actual presented
browser frame with bounded counts and reason.
