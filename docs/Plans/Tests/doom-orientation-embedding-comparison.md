...
| Status | Structural comparison running; no active provider change |
...
the candidate; use-driven dynamic doors remain disabled rather than being
silently exercised against the old frame.
...
- [x] Right/front and left/back asymmetric U-axis behavior under both
      candidates.
- [ ] Canonical `EXITSIGN` structural and native/browser visual regression.
...
- [ ] Independent non-Doom evidence capable of selecting east/X versus
      north/Z, or an explicit finding that Doom-local policy must choose.
...
## Provisional Architectural Result
