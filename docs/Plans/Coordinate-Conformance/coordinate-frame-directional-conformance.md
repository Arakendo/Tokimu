...
[`coordinate-frame-ledger.md`](Evidence/coordinate-frame-ledger.md). The ledger keeps
...
[`doom-coordinate-conversion-evidence.md`](Evidence/doom-coordinate-conversion-evidence.md).
...
[`coordinate-frame-comparative-results.md`](Evidence/coordinate-frame-comparative-results.md).
...
- [x] Retain the canonical package/player-start report and reproduction command
      in
      [`doom-source-world-spatial-orientation-evidence.md`](Studies/doom-source-world-spatial-orientation-evidence.md).
...
- [x] Classify the finding as H1, H2, H3, or a combination before changing the
      Doom point, direction, heading, wall, UV, renderer, or input paths.
- [x] Re-run the existing asymmetric sidedef, camera-control, projection,
  ...
  - [x] Retain canonical E1M1, picking/collision, flat, and browser candidate
        observations.
    ...
    - [x] Retain dynamic-door evidence: Preserve East and Preserve North both
          rebuild the four opening `DOORTRAK` spans in their selected frame,
          suppress them on close, and reuse the same handles on reopen.
    - [x] Retain the live browser Preserve North `EXITSIGN` observation. Browser
          WebGPU presented `1823/1823` opaque draws with
          `camera=canonical-exitsign`, `embedding=preserve-north`, and the
          maintainer confirmed that the sign reads properly.
...
[`doom-orientation-embedding-comparison.md`](Studies/doom-orientation-embedding-comparison.md).

The retained classification is **H1 + H2**. The old direct lift reverses the
canonical landmark determinant before a camera exists, and its lifted
source-right opposes the observer's camera-right. H3 explains why the first
free-camera screenshots were not sufficient evidence, but the fixed source
spawn fixture reproduces the signed mismatch and therefore eliminates H3 as
the cause of the retained invariant.
