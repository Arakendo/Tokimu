...
## 2026-08-13 Doom Reopening Closeout

Later canonical E1M1 evidence refined, but did not overturn, that disposition.
The old direct `(doom_x, height, doom_y)` lift was exactly invertible yet
orientation-reversing. The Doom corpus consumer now applies the explicit
Preserve North adapter (`east -> -X`, `north -> +Z`) to static geometry,
source-derived cameras and movement, conservative membership bounds, flat and
wall UV migration, and dynamically rebuilt door spans.

Preserve East passes the same Doom-relative controls and differs only by a
180-degree world-Y rotation. Therefore Preserve North is a declared Doom
consumer convention, not a universal Tokimu axis contract. Native controls and
Browser WebGPU agree on the canonical `EXITSIGN`; the final browser observation
presented `1823/1823` opaque draws with `embedding=preserve-north`, and the
maintainer confirmed readable `EXIT` art.

## Evidence Comparison

| Evidence family | Observed agreement | Owner of directional meaning | What it does not prove |
| --- | --- | --- | --- |
| Doom E1M1 and synthetic sidedefs | Readable right/front and left/back art, stable winding/normals, orientation-preserving reversible consumer lift, deterministic spawn commands and dynamic-door rebuilds | Doom provider retains source facts and side axes; corpus consumer owns the explicit Preserve North adapter and observer commands | Doom's chosen cardinal alignment is Tokimu's global world basis |
...
| --- | --- | --- |
| Doom `(x,y,height)` to corpus world coordinates | Explicit source-consumer conversion | Preserve North adapter in the Doom corpus consumer; the unchanged provider remains source authority |
