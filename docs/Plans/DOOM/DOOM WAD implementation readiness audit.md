...
| WAD code | `corpus/lib/doom-wad-provider` inspects `IWAD`/`PWAD` headers, ordered lump directories, and paired flat/patch/sprite marker ranges. `corpus/campaigns/doom/hello-wad-inspect` is its headless caller. Neither depends on Doom map, renderer, Resource Space, or gameplay APIs. | Slice 1 complete; no reviewed package is read by tests. |
